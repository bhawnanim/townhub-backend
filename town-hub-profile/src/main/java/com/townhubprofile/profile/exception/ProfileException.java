package com.townhubprofile.profile.exception;
...
@ControllerAdvice
public class ProfileException extends ResponseEntityExceptionHandler {

    private static Logger logger = LoggerFactory.getLogger(ProfileException.class);

    @ExceptionHandler({ Exception.class })
    private ResponseEntity<Result<String>> exception(Exception ex) {
        List<Result.TownHubError> error = new ArrayList<>();
        if (ex.getCause() == null) {
            error.add(new Result.TownHubError((ex.getMessage().hashCode()),
                    "there is some problem in the inbuilt functionality"));
            logger.error("\n" + ex.hashCode() + "\n" + ex.getCause().getMessage() + "\n");
        } else {
            error.add(new Result.TownHubError((ex.getMessage().hashCode()), ex.getCause().getMessage()));
        }
        logger.error("\n" + ex.hashCode() + "\n" + ex.getMessage());
    ...
    protected ResponseEntity<Object> handleTypeMismatch(TypeMismatchException ex, HttpHeaders headers,
        ...
        return new ResponseEntity<>(
                new Result<>(400, "Error! unable to parse the given data",
                        new ArrayList<>(
                                Arrays.asList(new Result.TownHubError(ex.hashCode(), ex.getLocalizedMessage())))),
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
        ...
        logger.error("\n" + ex.hashCode() + " ---- " + ex.getMessage());
        List<Result.TownHubError> errors = new ArrayList<>();
        for (FieldError error : ex.getBindingResult().getFieldErrors()) {
            errors.add(new Result.TownHubError(error.hashCode(), error.getDefaultMessage(), error.getField()));
        }
